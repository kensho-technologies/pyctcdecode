...
class TestLanguageModelSerialization(TempfileTestCase):
    ...
    def test_load_from_hub_offline(self):
        ...
        # get paths
        kenlm_file = os.path.join(self.temp_dir, kenlm_filename)
        attrs_file = os.path.join(self.temp_dir, attrs_filename)
        unigrams_file = os.path.join(self.temp_dir, unigrams_filename)

        # rename to hash
        os.rename(kenlm_file, os.path.join(self.temp_dir, url_to_filename(hf_url.format(dummy_hub_name, kenlm_filename))))
        os.rename(attrs_file, os.path.join(self.temp_dir, url_to_filename(hf_url.format(dummy_hub_name, attrs_filename))))
